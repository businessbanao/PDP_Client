...
export class DocManagementService {
  ...
  getFolderDocs(folderId: String): Observable<any> {
    ...
    return this._http.get(environment.baseUrl2 + `/api/v1/DocRecords?&folder_id=`+folderId).pipe(
      tap(
  ...
  deleteDoc(docId: String): Observable<any> {
    return this._http.delete(environment.baseUrl2 +'/api/v1/deletedoc/' + docId).pipe(
      tap(
        response => {
  ...
  updateDoc(docId, payload): Observable<any> {
    return this._http.put(environment.baseUrl2 +'/api/v1/editdoc/' + docId, payload).pipe(
      tap(
